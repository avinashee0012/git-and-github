...
public class TaskController {
    ...
    public ResponseEntity<Task> markTaskDone(@PathVariable int taskId){
        if(taskId <= 0) return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
        return ResponseEntity.status(HttpStatus.OK).body(taskService.markTaskDone(taskId));
    }
}
